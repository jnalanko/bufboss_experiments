...
outdir = experiment_dir + "/bifrost_out"
run("mkdir -p " + outdir)
...
resultdir = experiment_dir + "/bifrost_results"
...
if run_build:
    run_to_files("/usr/bin/time -v " + program + " build -r " + config.buildfile + " -k 30 -y -o " + built, resultdir + "/build")
if run_add:    
    run_to_files("/usr/bin/time -v " + program + " update -g " + built+".gfa" + " -r " + config.addfile + " -k 30 -o " + added, resultdir + "/add")
if run_query:
