...
//		BOOST_LOG_TRIVIAL(info) << "Adding kmer " << get_kmer_str(kmer, this->k) << "...";
	...
	void removeNode(const kmer_t& kmer) {
		...
		if (!this->detect_membership(kmer)) cerr<<"not here"<<endl;
		assert (this->detect_membership(kmer));
  ...
  bool IsEdgeInGraph(const kmer_t& u, const kmer_t& v) {
   ...
   bool newDynamicAddEdge( const kmer_t& u, const kmer_t& v ) {
	    ...
	    cerr << get_kmer_str(u, 29) << endl;
      cerr << get_kmer_str(v, 29) << endl;
      cerr << "this->k: " << this->k << endl;
		  cerr<<"impossible"<<endl;	return false;
		}
