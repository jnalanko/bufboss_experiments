...
int main(int argc, char* argv[]) {
    ...
    while (!sr.done()) {
        ...
        if(seq.size() >= edgemer_k){
            ...
            uint64_t KR_u = Graph.f.generate_KRHash_val_mod(nodemer_u, nodemer_k);
            ...
            for (long long i = nodemer_k; i < (long long)seq.size(); i++) {
                ...
                uint64_t KR_v = Graph.f.update_KRHash_val_OUT_mod(KR_u, access_kmer(nodemer_u,nodemer_k,0), access_kmer(nodemer_v,nodemer_k,nodemer_k-1));
